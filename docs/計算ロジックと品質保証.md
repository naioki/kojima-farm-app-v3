...
- **モジュール**: `box_remainder_calc.py`
  ...
  - **受信方法に基づく計算**: `calculate_inventory(input_num, master_unit, receive_as_boxes, unit_override=None)` → `(total, boxes, remainder, unit_used)`。マスタの「受信方法」が総数なら input_num=合計数量、箱数なら input_num=箱数。バラで「100本×7」など入数明記時は unit_override を使用。
...
| `order_processing.py` | 解析後の箱数・端数計算、補正（受信方法で分岐） | `calculate_inventory`, `total_to_boxes_remainder` |
...
- 解析後は `_compute_from_input_num_by_reception(entries)` で、マスタの **受信方法**（`receive_as_boxes`）に基づき計算する。
- **総数**: 「×」の後 = 合計数量。`calculate_inventory(input_num, master_unit, False)` で total=input_num, boxes=total//unit, remainder=total%unit。
- **箱数**: 「×」の後 = 箱数。`calculate_inventory(input_num, master_unit, True)` で boxes=input_num, total=boxes×unit, remainder=0。
- **バラの数値指定**: 規格バラで「100本×7」のように入数が明記されている場合は AI が `unit_from_text:100, input_num:7` を返し、`unit_override=100` で total=700 を算出。
